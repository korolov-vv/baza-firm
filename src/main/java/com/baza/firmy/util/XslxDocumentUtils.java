...
public class XslxDocumentUtils {
  ...
  public ByteArrayInputStream appendToExcel(ByteArrayOutputStream out, List<JdgListDto> jdgList, boolean isFirstPage, boolean isLastPage) {
    try (Workbook workbook = isFirstPage ? new XSSFWorkbook() : WorkbookFactory.create(new ByteArrayInputStream(out.toByteArray()))) {
      ...
      String[] columns = {"Nazwa", "NIP", "REGON", "Data rozpoczęcia działalności", "Email", "Telefon", "PKD główny", "PKD dodatkowe", "Adres Korespondencyjny", "Adres Działalności"};
      ...
      for (JdgListDto jdg : jdgList) {
        ...
        createCell(row, 2, jdg.getWlasciciel().getRegon(), cellStyle);
        ...
        createCell(row, 6, jdg.getPkdGlowny(), cellStyle);
        createCell(row, 7, String.join(", ", jdg.getPkd()), cellStyle);
        createCell(row, 8, jdg.getAdresKorespondencyjny().toString(), cellStyle);
        createCell(row, 9, jdg.getAdresDzialalnosci().toString(), cellStyle);
      }
